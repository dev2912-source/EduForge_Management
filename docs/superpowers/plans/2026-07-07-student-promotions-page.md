# Student Promotions Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the full promotions page matching demo.kedarkul.com/admin/students/promotions with per-student Transfer/Graduate/Skip actions.

**Architecture:** Single frontend page at `/dashboard/promotions` with 2-column layout (left: source/target/summary panels, right: student table). Backend enhancements add a `bulk-graduate` endpoint and extend the existing promotions endpoint with sections support.

**Tech Stack:** Next.js 16 (app router) + Tailwind CSS v4 + Lucide React icons + Express 5 + Mongoose 9

## Global Constraints

- All API calls use `Authorization: Bearer ${localStorage.getItem("token")}` header
- Models use `name` field, profiles use nested `profile.*` fields
- Existing backend routes in `backend/routes/adminRoutes.js` follow the same route handler pattern
- AcademicHistory records must be created for Transfer and Graduate actions
- No external chart/dependency libraries — use inline SVG/Tailwind
...
|------|--------|----------------|
| `frontend/src/app/dashboard/promotions/page.jsx` | Rewrite | Complete promotions page UI |
| `backend/routes/adminRoutes.js` | Modify | Add `POST /students/bulk-graduate`, enhance `POST /promotions` |

---

### Task 1: Backend — Add bulk-graduate endpoint

**Files:**
- Modify: `backend/routes/adminRoutes.js` (add before the `module.exports` block near line 1183)

**Interfaces:**
- Consumes: `req.body` = `{ studentIds: string[], academicYear: string, remarks?: string }`
- Produces: `POST /api/admin/students/bulk-graduate` route handler

- [ ] **Add bulk-graduate route before `module.exports`**
...
- [ ] **Verify backend parses correctly**

Run: `cd backend && node -e "require('./routes/adminRoutes'); console.log('OK')"`
Expected: `OK`

---

### Task 2: Backend — Enhance promotions endpoint with toSection + remarks

**Files:**
- Modify: `backend/routes/adminRoutes.js` (lines 1082-1105)

**Interfaces:**
- Consumes: enhanced `req.body` now includes `toSection` and `remarks` fields
- Produces: promotions endpoint now accepts `toSection` and writes `remarks` to AcademicHistory

- [ ] **Update POST /promotions to accept toSection and remarks**

Replace lines 1082-1084:
...
Then replace the AcademicHistory creation inside the loop (around line 1094-1101):
...
Also update the bulk promotions section (else-if block at line 1109) to use the same changes — replace section assignment and remarks:
...
### Task 3: Frontend — Rewrite promotions page

**Files:**
- Rewrite: `frontend/src/app/dashboard/promotions/page.jsx`

**Interfaces:**
- Consumes: APIs at `GET /api/admin/classes`, `GET /api/admin/students?className=X&section=Y&status=active&limit=500`, `POST /api/admin/promotions`, `POST /api/admin/students/bulk-graduate`
- Produces: Full promotions page at `/dashboard/promotions`

- [ ] **Write the complete promotions page**

Replace the entire `frontend/src/app/dashboard/promotions/page.jsx` with:
...
Expected: compiled successfully, route `/dashboard/promotions` listed
...
Run: `cd frontend && npm run build`
Expected: "Compiled successfully", 32+ routes listed including `/dashboard/promotions`
