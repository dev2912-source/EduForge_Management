# Fee Structures Page — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite the fee structures page with a polished UI matching the admin pages pattern, and add CSV/Excel import.

**Architecture:** Single-page Next.js app router page with inline modals (same pattern as classes/promotions/students pages). One new backend import endpoint via multer.

**Tech Stack:** Next.js 16, Tailwind CSS v4, Express 5, Mongoose 9, Lucide React icons, multer
...
- Pagination per-page stored in localStorage key `feeStructuresPerPage` (default 10)
- FeeStructure model: `{ className, academicYear, category, amount, frequency: ENUM['Monthly','Quarterly','Annually','Bi-Annually','One-Time'], dueDay, timestamps }`
- Frequency badges: Monthly → `bg-blue-50 text-blue-700`, Quarterly → `bg-purple-50 text-purple-700`, Annually → `bg-green-50 text-green-700`, One-Time → `bg-orange-50 text-orange-700`, Bi-Annually → `bg-stone-50 text-stone-700`
- Currency format: `Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' })`
- API base: `http://localhost:5000`
- No browser `confirm()` — use modal for delete confirmation
- Backend endpoint: `DELETE /api/admin/fee-structures/:id` returns `{ success, message }`

---

### Task 1: Add POST /api/admin/fee-structures/import backend endpoint

**Files:**
- Modify: `backend/routes/adminRoutes.js` — add after the existing GET/POST/PUT/DELETE fee-structures routes (around line 970)

**Interfaces:**
- Consumes: FeeStructure model, multer for file upload
- Produces: `POST /api/admin/fee-structures/import` accepting multipart CSV/Excel, returning `{ success, count, errors }`

- [ ] **Step 1: Verify existing fee-structures routes end around line 970**

Read `backend/routes/adminRoutes.js` lines 955-980 to find the end of the DELETE fee-structures handler.

```bash
cd E:\ASTNIQ-SOLUTION\KedarKul
Select-String -LiteralPath backend/routes/adminRoutes.js -Pattern "fee-structures" | Select-String -NotMatch "// @"
```

Expected: Shows the 4 CRUD handlers for fee-structures (GET, POST, PUT, DELETE).

- [ ] **Step 2: Add the import endpoint after the DELETE handler**

Insert after the closing `});` of the DELETE `/fee-structures/:id` handler:
...
**IMPORTANT:** This route MUST be placed BEFORE `/fee-structures/:id` routes (PUT and DELETE) so Express does not match "import" as an `:id`. Insert it right after the POST `/fee-structures` handler, before the PUT handler.
...
git commit -m "feat: add fee structures import endpoint"
```

---

### Task 2: Rewrite fee structures page with polished UI

**Files:**
- Rewrite: `frontend/src/app/dashboard/finance/structures/page.jsx` (428 lines → ~550 lines)

**Interfaces:**
- Consumes: `GET /api/admin/fee-structures?page=X&limit=Y&search=Z` → `{ success, data, total, page, pages }`
- Consumes: `POST /api/admin/fee-structures` with body `{ className, academicYear, category, amount, frequency, dueDay }`
- Consumes: `PUT /api/admin/fee-structures/:id` with same body
- Consumes: `DELETE /api/admin/fee-structures/:id`
- Consumes: `POST /api/admin/fee-structures/import` with multipart file
- Consumes: `GET /api/admin/classes?limit=1000` for class select options

- [ ] **Step 1: Write the full rewrite of the fee structures page**
...
Expected: Build succeeds with no errors. Check for `/dashboard/finance/structures` in the route list.

- [ ] **Step 3: Commit**

```bash
cd E:\ASTNIQ-SOLUTION\KedarKul
git add frontend/src/app/dashboard/finance/structures/page.jsx
git commit -m "feat: rewrite fee structures page with polished UI and import"
```
