...
export default function MyFeesPage() {
  ...
  const totalPaid = invoices.reduce((acc, inv) => acc + (parseInt((inv.paidAmount || '0').replace(/[^0-9]/g, ''), 10) || 0), 0);
  const totalDue = invoices.reduce((acc, inv) => acc + (parseInt((inv.balance || inv.amount).replace(/[^0-9]/g, ''), 10) || 0), 0);

  const totalPaidStr = '₹' + totalPaid.toLocaleString();
  const totalDueStr = '₹' + totalDue.toLocaleString();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white p-4 rounded-2xl border border-stone-200 shadow-sm flex-shrink-0">
        <div className="space-y-0.5">
          ...
          <p className="text-sm text-stone-500 font-medium ml-3.5">
            View and download your fee invoices
          </p>
        </div>
        <div className="flex items-center gap-3 flex-wrap text-xs font-bold">
          <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-50 text-red-700 border border-red-100">
      ...
      <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden flex-1 flex flex-col min-h-0">
        ...
        <div className="px-4 py-3 border-b border-stone-100 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="relative group">
            ...
            <input 
              className="block rounded-lg border border-stone-200 bg-white pl-9 pr-4 py-1.5 text-sm text-stone-800 shadow-sm transition-all focus:outline-none focus:border-[#F97316] focus:ring-2 focus:ring-[#F97316]/20 w-44 sm:w-56" 
          ...
          <table className="w-full min-w-[640px]">
            <thead className="sticky top-0 bg-white z-10 shadow-sm">
              <tr className="border-b border-stone-200">
                ...
                <th className="hidden md:table-cell text-left py-3 px-4">
                  <span className="text-sm font-bold text-stone-700">Paid</span>
                </th>
                <th className="hidden sm:table-cell text-left py-3 px-4">
                  <span className="text-sm font-bold text-stone-700">Balance</span>
                </th>
                <th className="hidden sm:table-cell text-left py-3 px-4">
                  <button className="flex items-center gap-1.5 group text-sm font-bold text-stone-700 hover:text-stone-900 transition-colors">
                    Due Date
            ...
            <tbody className="divide-y divide-stone-50">
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="py-8 text-center text-stone-400">No invoices found.</td>
                </tr>
              ) : invoices.map((inv, idx) => (
                <tr key={idx} className="group transition-all duration-100 cursor-pointer hover:bg-stone-50/70">
                  <td className="py-2.5 px-4">
                    <span className="font-mono text-xs font-bold text-stone-800">{inv.invoiceId || inv.id}</span>
                  </td>
                  <td className="hidden sm:table-cell py-2.5 px-4">
                    <span className="text-sm font-bold text-stone-700">{inv.amount}</span>
                  </td>
                  <td className="hidden md:table-cell py-2.5 px-4">
                    <span className="text-sm font-bold text-green-700">{inv.paidAmount || '₹0'}</span>
                  </td>
                  <td className="hidden sm:table-cell py-2.5 px-4">
                    <span className={`text-sm font-bold ${(inv.status || 'Pending').toLowerCase() === 'paid' ? 'text-green-700' : 'text-red-600'}`}>
                      {inv.balance || inv.amount}
                    </span>
                  </td>
                  <td className="hidden sm:table-cell py-2.5 px-4">
                    <span className="text-sm text-stone-600">{inv.dueDate}</span>
                  </td>
                  <td className="py-2.5 px-4">
                    <span className={`
                      text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md border
                      ${(inv.status || 'Pending').toLowerCase() === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-100' : ''}
                      ${(inv.status || 'Pending').toLowerCase() === 'partial' ? 'bg-blue-50 text-blue-700 border-blue-100' : ''}
                      ${(inv.status || 'Pending').toLowerCase() === 'paid' ? 'bg-green-50 text-green-700 border-green-100' : ''}
                    `}>
                      {inv.status || 'Pending'}
                    </span>
                  </td>
                  <td className="text-right py-2.5 px-4">
                    <div className="flex items-center justify-end gap-2">
                      <button className="p-1.5 rounded-lg text-stone-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Preview PDF">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button className="p-1.5 rounded-lg text-stone-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors" title="Download PDF">
        ...
        <div className="px-4 py-3 border-t border-stone-100 bg-stone-50/30 flex items-center justify-between gap-3 flex-shrink-0">
          <span className="text-xs font-bold text-stone-500 whitespace-nowrap">
            1–{invoices.length} of <span className="text-stone-800 font-black">{invoices.length}</span>
          </span>
