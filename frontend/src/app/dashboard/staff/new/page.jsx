...
export default function NewStaffPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const staffId = data.data?.id;
            if (photo && staffId) {
                const fd = new FormData();
                fd.append('photo', photo);
                await fetch(`http://localhost:5000/api/admin/staff/${staffId}/photo`, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` },
                    body: fd
                }).catch(() => {});
        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto space-y-4">
            ...
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-base font-bold text-stone-900">New Staff</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {step === 2 && (
                    <>
                        ...
                        {/* Portal Credentials (New only) */}
                        <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6 space-y-5">
                            <div className="flex items-center gap-2 border-b border-stone-100 pb-3">
                                <div className="w-1.5 h-4 rounded-full bg-orange-500" />
                                <h2 className="text-sm font-black text-stone-800 uppercase tracking-widest">Portal Credentials</h2>
                            </div>

                            <div className="flex items-start gap-2.5 p-3 bg-blue-50 rounded-lg border border-blue-100">
                                <svg className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <p className="text-[10px] text-blue-700 font-bold leading-relaxed">
                                    Login ID is auto-assigned as the staff code (e.g. STF-2024-0001). No separate username needed.
                                </p>
                            </div>

                            <div className="max-w-xs">
                                <label className="form-label text-[10px] uppercase tracking-wider font-black text-stone-400">Password</label>
                                <input
                                    type="text"
                                    value={form.password}
                                ...
                                <p className="text-[10px] text-stone-400 font-bold mt-1">
                                    {form.password ? 'The specified password will be set.' : 'A 10-character temporary password will be auto-generated and shared.'}
                                </p>
                            </div>
                        </div>
                    </>
            ...
            {showCredentialModal && credentialData && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-xl border border-stone-200 w-full max-w-md m-4 p-6">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-lg font-black text-stone-900">Staff Created</h3>
                            <button onClick={() => { setShowCredentialModal(false); router.push('/dashboard/staff'); }} className="p-1 rounded-md text-stone-400 hover:text-stone-600 hover:bg-stone-100 transition-colors">
                                <X size={18} />
                            </button>
                        </div>
                        <div className="space-y-3 mb-5">
                            <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg border border-green-100">
                                ...
                                <span className="text-xs font-bold text-green-700">{credentialData.name} has been added successfully.</span>
                            </div>
                            <div className="text-xs space-y-2 font-bold text-stone-600">
                                <p><span className="text-stone-400">Login ID:</span> <span className="text-stone-800 font-mono">{credentialData.loginId}</span></p>
                                {credentialData.email && <p><span className="text-stone-400">Email:</span> <span className="text-stone-800">{credentialData.email}</span></p>}
                                <p className="text-stone-400 font-medium" style={{ whiteSpace: 'pre-wrap' }}>Share these credentials with the staff member. They can log in using their Login ID.</p>
                            </div>
                        </div>
                        <button
                            onClick={() => { setShowCredentialModal(false); router.push('/dashboard/staff'); }}
                            className="w-full py-2.5 text-xs font-black uppercase tracking-widest rounded-xl bg-stone-900 text-white hover:bg-stone-800 transition-all"
                        >
