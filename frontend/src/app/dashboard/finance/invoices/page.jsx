...
export default function FeeInvoicesPage() {
    ...
    const fetchInvoices = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/admin/invoices?page=${page}&limit=${limit}&search=${search}`, {
    ...
    const getStatusBadge = (status) => {
        ...
        if (s === 'paid') {
            return <span className="px-2 py-0.5 rounded text-[11px] font-black uppercase tracking-wide bg-green-50 text-green-700 border border-green-200">Paid</span>;
        }
        if (s === 'overdue') {
            return <span className="px-2 py-0.5 rounded text-[11px] font-black uppercase tracking-wide bg-red-50 text-red-700 border border-red-200">Overdue</span>;
        }
        return <span className="px-2 py-0.5 rounded text-[11px] font-black uppercase tracking-wide bg-orange-50 text-orange-600 border border-orange-200">{status || 'Pending'}</span>;
    };
            ...
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
                ...
                <div className="px-3 py-3 border-b border-stone-100 flex items-center justify-end gap-2 bg-white">
                    <div className="relative group">
                        ...
                        <input 
                            type="text" 
                            className="block rounded-lg border border-stone-200 bg-white pl-8 pr-7 py-1.5 text-sm text-stone-800 shadow-sm transition-all focus:outline-none focus:ring-2 w-56" 
                    ...
                    <table className="w-full min-w-[900px]">
                        <thead>
                            <tr className="bg-white border-b border-stone-200 text-left">
                                ...
                                <th className="py-4 px-3">
                                    <button className="flex items-center gap-1.5 group text-xs font-bold text-stone-700 hover:text-stone-900 transition-colors">
                                        Paid <ChevronDown size={12} className="text-stone-400 opacity-0 group-hover:opacity-100" strokeWidth={3} />
                                    </button>
                                </th>
                                <th className="py-4 px-3">
                                    <button className="flex items-center gap-1.5 group text-xs font-bold text-stone-700 hover:text-stone-900 transition-colors">
                                        Balance <ChevronDown size={12} className="text-stone-400 opacity-0 group-hover:opacity-100" strokeWidth={3} />
                                    </button>
                                </th>
                                <th className="py-4 px-3">
                                    <button className="flex items-center gap-1.5 group text-xs font-bold text-stone-700 hover:text-stone-900 transition-colors">
                                        Due Date <ChevronDown size={12} className="text-stone-400 opacity-0 group-hover:opacity-100" strokeWidth={3} />
                                    </button>
                                </th>
                                <th className="py-4 px-3">
                                    <button className="flex items-center gap-1.5 group text-xs font-bold text-stone-700 hover:text-stone-900 transition-colors">
                                        Status <ChevronDown size={12} className="text-stone-400 opacity-0 group-hover:opacity-100" strokeWidth={3} />
                                    </button>
                                </th>
                                <th className="py-4 px-3">
                                    <button className="flex items-center gap-1.5 group text-xs font-bold text-stone-700 hover:text-stone-900 transition-colors">
                        ...
                        <tbody className="divide-y divide-stone-50">
                            {loading ? (
                                <tr><td colSpan="9" className="py-6 text-center text-sm text-stone-500">Loading invoices...</td></tr>
                            ) : invoices.length === 0 ? (
                                <tr><td colSpan="9" className="py-6 text-center text-sm text-stone-500">No invoices found</td></tr>
                            ) : (
                                invoices.map((invoice) => (
                                    <tr key={invoice._id} className="group transition-all duration-100 cursor-pointer hover:bg-stone-50/70">
                                        ...
                                        <td className="py-3 px-3">
                                            <span className="text-sm font-black text-stone-800">{invoice.balance}</span>
                                        </td>
                                        <td className="py-3 px-3">
                                        ...
                                        <td className="py-3 px-3">
                                            <span className="text-xs text-stone-400 font-medium whitespace-nowrap">{formatDateTime(invoice.createdAt)}</span>
                                        </td>
                                    </tr>
                                ))
                            )}
